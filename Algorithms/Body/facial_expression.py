...
while True:
    ...
    if not ret:  # Check if the frame is captured correctly
    ...
    faces = face_cascade.detectMultiScale(gray_frame, scaleFactor=1.1, minNeighbors=30, minSize=(30, 30))

    for (x, y, w, h) in faces:
        # Extract the face ROI (Region of Interest)
        face_roi = frame[y:y + h, x:x + w]  # Use the original frame for DeepFace
